...
- **Cross-artifact consistency**: a constraint declared in one artifact (a Mongoose schema's `maxlength`/`minlength`/`enum`/`required`/`match`, a type definition, a documented business rule) that is not mirrored by every other artifact that should enforce or describe it — most commonly an HTTP-layer validation schema (Zod) not replicating a persistence-layer constraint on the same field, or the reverse. Also check the inverse direction: a code comment, JSDoc block, `docs/api-contract.md` entry, or Swagger `description` that asserts a specific behavior — re-derive that behavior from the actual code path it describes and flag any mismatch, rather than trusting the prose. This category is not caught by ESLint or SonarCloud (there is no static rule for "declared constraint not enforced elsewhere" or "comment doesn't match code") and has been the source of real external findings this review missed (e.g. a schema-declared field length not enforced by request validation; a JSDoc claim about deletion-order guarantees that didn't match the actual persistence behavior) — treat it with the same seriousness as the other items here, not as an afterthought.
