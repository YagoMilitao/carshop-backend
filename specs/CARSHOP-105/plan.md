...
- **express / body-parser**: `src/infra/config/middleware.ts` (`express.json({limit:'1mb'})`), `src/infra/server.ts`, all route builders in `src/infra/http/routes/*.routes.ts`, and critically `src/infra/presentation/middleware/error-handler.middleware.ts` — confirmed still has the correct 4-arg `ErrorRequestHandler` signature (`error, _request, response, _next`), so the CARSHOP-104 arity bug is not currently regressed. `body-parser` ships bundled inside `express@5.x` as a transitive dependency; it cannot be upgraded independently without either (a) an express patch/minor that bumps its own `body-parser` pin, or (b) an `overrides` entry in `package.json` (the project already uses `overrides` for `path-to-regexp`/`picomatch`, an established, low-risk pattern here) — prefer an override over a manual express major bump if express itself doesn't need bumping.
